...
const Profile = () => {
    ...
    return (
        <div className='flex flex-col gap-6 w-[953px] my-7 mx-5 overflow-y-auto max-h-[100vh-122px] scroll-main dark:bg-gray-800 dark:text-white'>
            <div className='flex flex-col m-auto items-start sm:m-0 sm:flex-row '>
                ...
                <div className='mt-6 sm:ml-6 '>
                    <h1 className='text-xl sm:text-[25px] font-bold text-slate-800  dark:text-white'>{user?.fullName}</h1>
                    <p className=' text-zinc-600  dark:text-white'>{user?.email}</p>
            ...
            <div className='mt-10 '>
                <h3 className='text-xl sm:text-2xl font-bold text-zinc-700 dark:text-white'>Stats</h3>
                <div className='grid grid-cols-1 mx-1 sm:grid-cols-2 gap-4 md:grid-cols-3 mt-10 '>
                    {statsData.map(stat => (
                        <div key={stat.label} className='shadow flex items-center rounded-lg bg-white p-4 dark:bg-gray-700 dark:text-white'>
                            <i className={`fa-solid ${stat.icon} text-xl mr-4 text-zinc-700  dark:text-white`}></i>
                            <div>
                                <p className='text-[16px] font-bold sm:text-xl text-zinc-700  dark:text-white'>{stat.label}</p>
                                <span className='text-xl font-semibold sm:text-3xl text-zinc-700  dark:text-white'>{stat.value}</span>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <h3 className='mt-5 text-xl sm:text-2xl font-bold text-zinc-700  dark:text-white'>Badges</h3>
            <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4'>
                {badgesData.map(badge => (
                    <div key={badge.label} className={`flex items-center p-4 rounded-lg ${badge.color} `}>
                        ...
                        <div className='ml-3 '>
                            <p className='text-xl sm:text-2xl font-bold dark:text-gray-800'>{badge.label}</p>
                            <span className='text-xl font-bold sm:text-2xl dark:text-gray-800'>{badge.value || 0}</span>
                        </div>
                    </div>
                ))}
            </div>
